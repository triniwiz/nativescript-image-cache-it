	...
	<Page>
		...
		<GridLayout columns="*" rows="*">
			<ListView for="item in items">
				<v-template>
					<GridLayout class="list-group-item" columns="auto,*" [nsRouterLink]="['/item', item.id]"
								[text]="item.name">
						<ImageCacheIt placeHolder="res://place" errorHolder="res://error" stretch="fill" width="100"
									  height="100"
									  :src="item.url"/>
						<StackLayout col="1">
							<Label :text="item.url"/>
